...
- **Reminders**: a list in the lower half of the sidebar (drag the separator to resize), soonest first, grouped under **Overdue / Today / This week / Next week / Later**. A reminder is due on a *day* — there is no time of day, and nothing pops up or interrupts: today's entries simply stand out in the list, and the badge counts what is due now. Add one with `＋`, or right-click a file in the tree. A reminder happens once — it does not repeat — belongs to a **list** (`todo` by default, and typing any other name in the editor creates it), and can link to a note. The dropdown in the Reminders header filters the pane down to one list, and remembers the choice. Right-click a row to extend it by a day, three days, a week or a month, open its note, or mark it done. The list is stored as plain JSON in `.wisp-reminders.json` at the vault root.
...
| Filter reminders by list | The dropdown in the Reminders header |
| Edit a reminder | Click it in the list (right-click for extend / open / complete / delete) |
| Finish a reminder | `✓` on hover, or right-click → *Done* |
