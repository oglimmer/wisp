...
- **Prompt inlines small files.** `gatherFiles` embeds the contents of files under the size/total budget directly in the prompt so Claude usually decides in **one turn without `Read` round-trips** (large files are listed by name and read on demand). This is the difference between ~7s and Claude crawling the vault. **Only text is inlined**, through the same `isBinaryBuffer()` NUL heuristic the diff view uses (in `main/guards.mts`, so there is one of it): a vault holds pictures and a small one is well under the budget, so decoding every file as UTF-8 put a PNG's bytes into the prompt — and a NUL is a character `spawn()` refuses in an argument, so a single imported image failed *every* smart insert and lookup in that vault with a TypeError. A binary file is listed by name like an oversized one.
