import type { WispApi } from './types/ipc';

// This file is CommonJS and has to stay that way — a sandboxed preload cannot be
// ESM. `require` rather than `import` is what keeps the emitted preload.js
// CommonJS under `module: preserve`, which preserves each file's module syntax as
// written; an `import` here would emit an `import` and Electron would refuse to
// load it. The annotation is what buys electron's own types back, since a bare
// `require` returns `any`.
const { contextBridge, ipcRenderer, webUtils }: typeof import('electron') = require('electron');

// Safe, minimal API exposed to the renderer.
//
// Typed against the shared contract in `types/ipc.d.ts`, which is also what
