...
function refToRelPath(ref: string) {
...
export function resolveVaultRef(
  ...
  noteDir: string,
  ref: string,
  exists: (abs: string) => boolean = fs.existsSync,
): { abs: string; style: 'note' | 'root' } | null {
  const rel = refToRelPath(ref);
  if (rel === null) return null;
  const base = path.resolve(baseFolder);
  const candidates = [
    { abs: path.resolve(noteDir, rel), style: 'note' as const },
    { abs: path.resolve(base, rel), style: 'root' as const },
  ];
...
function refFor(baseFolder: string, noteDir: string, target: string, style: 'note' | 'root') {
...
function rewriteMovedRefs(
  baseFolder: string,
  text: string,
  noteOldDir: string,
  noteNewDir: string,
  mapTarget: (abs: string) => string,
) {
  let changed = 0;
  const out = text.replace(MD_REF_RE, (whole: string, open: string, ref: string, tail: string) => {
...
async function gatherTextFiles(baseFolder: string) {
  const out: string[] = [];
