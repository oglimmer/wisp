...
async function imageDest(baseFolder: string, nameHint: string, ext: string) {
...
// the vault moves.
function refFor(baseFolder: string, currentFile: string | null, dest: string) {
  const fromDir = currentFile ? path.dirname(currentFile) : baseFolder;
...
// Stated rather than inferred: the caller reads `decoded.ext`/`decoded.buf` after
// checking `decoded.ok`, and TypeScript widens `ok: false` in an object literal to
// `boolean` unless the union is declared — which would leave nothing to narrow on.
type DecodedImage = { ok: true; buf: Buffer; ext: string } | { ok: false; error: string };
...
function decodeImageDataUrl(dataUrl: string): DecodedImage {
  ...
  // Annotated, not inferred: as a bare `const` the `ok: false` widens to `boolean`
  // and the value stops being assignable to the union it is returned as.
  const tooBig: DecodedImage = {
    ok: false,
    error: `Image is too large (max ${formatBytesLimit(MAX_IMAGE_BYTES)}).`,
  };
