...
import type { WindowState } from './config.mjs';
...
function restoredWindowState() {
  ...
  const num = (v: unknown, fallback = 0) => (Number.isFinite(v) ? Math.round(v as number) : fallback);
...
export function createWindow(cleanupOnClose: () => void) {
  const state = restoredWindowState();

  // Held as a local as well as on the module binding: the listeners below close
  // over it, and a closure cannot narrow `BrowserWindow | undefined` down to a
  // window however obviously it was just assigned.
  ...
  // Listed rather than looped: `on` is a set of overloads, one per event name, and
  // a union of names matches none of them — a loop would need a cast that throws
  // away the checking these six lines get for free.
...
ipcMain.handle('choose-folder', async () => {
  ...
  // The modal-parent overload wants a window; without one the dialog is still
  // shown, just not attached to it.
