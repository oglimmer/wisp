...
export function pipePositions(line) {
  const at = [];
...
/** @type {{body: string, blocks: {prefix: string, blocks: {raw: string, sig: string}[]} | null} | null} */
let blockCache = null;
...
function paneBlocks(paneEl) {
  ...
  for (const node of Array.from(paneEl.childNodes)) {
    if (node.nodeType === Node.TEXT_NODE) {
      pending += escapeHtml(/** @type {Text} */ (node).data);
      continue;
    }
    ...
    if (node.nodeType === Node.COMMENT_NODE) {
      flush();
      out.push(paneBlock(`<!--${/** @type {Comment} */ (node).data}-->`));
      continue;
    }
