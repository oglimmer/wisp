...
// One step of a diff. `ctx`/`del`/`add` are what the LCS produces; `gap` is only
// ever added by condenseDiff, which replaces a long unchanged run with a marker —
// so a consumer that has not condensed will never see one.
export type OpType = 'ctx' | 'del' | 'add';
export interface Op {
  type: OpType;
  text: string;
}
export interface GapOp {
  type: 'gap';
  text: string;
}

export function lineDiff(a: string, b: string): (Op | GapOp)[] {
...
function commonUniqueLines(
  ...
  const once = (arr: string[], from: number, to: number) => {
    const at = new Map<string, number>(); // text -> its index, or -1 once a second copy is seen
...
function condenseDiff(ops: Op[]): (Op | GapOp)[] {
  const CONTEXT = 2;
