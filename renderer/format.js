...
function fenceBlocks(lines) {
  /** @type {{start: number, end: number, closed: boolean}[]} */
  const out = [];
  /** @type {{start: number, fence: string} | null} */
  let open = null;
...
function elementOf(node) {
  ...
  return el && el.nodeType === Node.ELEMENT_NODE ? /** @type {Element} */ (el) : null;
}
...
function liftOut(el) {
  ...
  while (node.parentNode && node.parentNode !== wysiwygEl && wysiwygEl.contains(node.parentNode)) {
    const parent = /** @type {Element} */ (node.parentNode);
    const grandparent = parent.parentNode;
    if (!grandparent) break;
    const tail = /** @type {Element} */ (parent.cloneNode(false));
...
function removeBlock(el) {
  ...
  while (node && node !== wysiwygEl) {
    ...
    /** @type {Element} */ (node).remove();
    if (!parent || parent === wysiwygEl || /** @type {Element} */ (parent).children.length) break;
    node = parent;
  }
}
