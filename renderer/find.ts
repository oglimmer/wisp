...
// Which one is in here depends on the mode, so anything reading it has to know
// which it is in — the union is the honest type, not a convenience.
interface TextMatch {
  start: number;
  end: number;
}
let findMatches: (TextMatch | Range)[] = [];

// Which of the two is in `findMatches` is decided by the pane, and every site
// below has already established that — by checking `effectiveViewMode()`, or by
// being reachable only from the branch that filled it. These say so once, rather
// than leaving an assertion at each use where the reasoning would be invisible.
const asTextMatches = () => findMatches as TextMatch[];
const asRanges = () => findMatches as Range[];
let findIndex = -1;
let findRefreshTimer: ReturnType<typeof setTimeout> | null = null;
...
function findInText(text: string, query: string): TextMatch[] {
  const out: TextMatch[] = [];
  if (!query) return out;
  let hay = text;
  let needle = query;
...
function findInDom(container: HTMLElement, query: string): Range[] {
...
function paintDomHighlights() {
  ...
  CSS.highlights.set(HL_ALL, new Highlight(...asRanges().filter((_, i) => i !== findIndex)));
  const current = asRanges()[findIndex];
...
function scrollToCurrentMatch() {
  ...
  if (effectiveViewMode() === 'raw') {
    const span = findHighlightsEl.querySelector('.find-hit.current') as HTMLElement | null;
    if (!span) return;
    ...
    const rect = (m as Range).getBoundingClientRect();
...
export function refreshFind(anchor?: unknown) {
  if (!findOpen) return;
  clearFindHighlights();
  findMatches = [];
  findIndex = -1;
  ...
  if (findMatches.length) {
    ...
    if (typeof anchor === 'number' && mode === 'raw') {
      const i = asTextMatches().findIndex((m) => m.start >= anchor);
      if (i !== -1) findIndex = i;
    }
  }
  paintFindHighlights();
...
export function closeFind() {
  ...
  if (mode === 'raw') {
    if (!editorEl.disabled) {
      editorEl.focus();
      if (match) editorEl.setSelectionRange((match as TextMatch).start, (match as TextMatch).end);
    }
...
function replaceCurrent() {
  ...
  const m = asTextMatches()[findIndex];
  if (!m) return;
